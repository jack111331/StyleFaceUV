...
class Trainer(TrainValidBaseTrainer):
    def init_fn(self):
        ...
        self.saver = BestFirstPoseScalarCheckpointSaver(self.options.ckpt_output_dir)
        if self.saver.checkpoint != None:
            additional_param = self.saver.load_checkpoint(models={"Stylecode to Pose Scalar": self.stylecode_to_scalar_model}, optimizers={"Stylecode to Pose Scalar Optimizer": self.optimizer})
        ...
        if debug_output == True:
            ...
            debug_output_dir = os.path.join(self.options.debug_output_dir, "pose_scalar")
    ...
    def post_valid_step(self, epoch, epoch_loss):
        ...
        if epoch == 0:
            ...
            self.saver.save_checkpoint(models={"Stylecode to Pose Scalar": self.stylecode_to_scalar_model}, optimizers={"Stylecode to Pose Scalar Optimizer": self.optimizer}, trainer=self)
            ...
            super(BestFirstCheckpointSaver, self.saver).save_checkpoint(models={"Stylecode to Pose Scalar": self.stylecode_to_scalar_model}, optimizers={"Stylecode to Pose Scalar Optimizer": self.optimizer},
                                       trainer=self)
            ...
            if epoch_loss < self.best_loss:
                ...
                self.saver.save_checkpoint(models={"Stylecode to Pose Scalar": self.stylecode_to_scalar_model}, optimizers={"Stylecode to Pose Scalar Optimizer": self.optimizer},
                                           trainer=self)

                # best_model_wts = copy.deepcopy(self.stylecode_to_scalar_model.state_dict())
                # torch.save(best_model_wts, os.path.join(self.options.ckpt_output_dir, "param.pkl"))
                scalar = self.stylecode_to_scalar_model(torch.flatten(self.valid_data, start_dim=1))
                scalar = scalar.view(-1, 1, 1)
                if debug_output == True:
...
class BestFirstPoseScalarCheckpointSaver(BestFirstCheckpointSaver):
    # inheritance
