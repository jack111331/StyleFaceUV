...
from .network.arch import Generator2D, Discriminator2D
...
if __name__ == "__main__":
    ...
    g = Generator2D(size, 512, n_mlp, channel_multiplier=args.channel_multiplier)
    ...
    if args.gen:
        g_train = Generator2D(size, 512, n_mlp, channel_multiplier=args.channel_multiplier)
        g_train_state = g_train.state_dict()
        g_train_state = fill_statedict(g_train_state, generator.vars, size, n_mlp)
        ckpt["g"] = g_train_state

    if args.disc:
        disc = Discriminator2D(size, channel_multiplier=args.channel_multiplier)
