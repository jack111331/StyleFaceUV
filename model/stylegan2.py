...
class Upsample(nn.Module):
    ...
    def forward(self, input):
        ...
        out = upfirdn2d.upfirdn2d(input, self.kernel, up=self.factor, down=1, padding=self.pad)
        return out
...
class EqualLinear(nn.Module):
    ...
    def forward(self, input):
        if self.activation:
            ...
            # I've checked its lrelu parameter match the new implementation
            out = bias_act.bias_act(out, self.bias * self.lr_mul, act='lrelu')
...
class FusedLeakyReLU(nn.Module):
    ...
    def forward(self, input):
        return bias_act.bias_act(input, self.bias, act='lrelu')
