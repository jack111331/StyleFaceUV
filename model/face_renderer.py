...
class FaceRenderer(nn.Module):
    ...
    # input: id_coeff with shape [batch, 80]
    #        ex_coeff with shape [batch, 64]
    # output: face_shape with shape [1, N, 3], N is number of vertices
    def shape_formation(self, id_coeff, ex_coeff):
        n_b = id_coeff.size(0)

        face_shape = torch.einsum('ij,bj->bi', self.idBase, id_coeff) + \
                     torch.einsum('ij,bj->bi', self.exBase, ex_coeff) + self.meanshape
