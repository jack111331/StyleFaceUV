...
class Trainer(GANBaseTrainer):
    def init_fn(self):
        ...
        self.stylecode_to_scalar_model.load_state_dict(torch.load(self.options.stylecode_pose_scalar_mlp_ckpt))
        ...
        self.stylecode_to_3dmm_coeff_model.load_state_dict(torch.load(self.options.stylecode_3dmm_coeff_mlp_ckpt))
    ...
    def train_step(self, idx, input_batch, stage):
        ...
        if idx % self.options.n_critic_d == 0:
            ...
            if self.d_cnt % self.options.d_reg_every == 0:
                ...
                #######
                self.discriminator.zero_grad()
                (10 / 2 * r1_loss * self.options.d_reg_every + 0 * real_pred[0]).backward()
                self.optimizer_d.step()
        if idx % self.options.n_critic == 0:
            ...
            loss = g_loss + self.options.photo_weight * (
                    stylerig_photo_loss_v + 0.2 * perceptual_loss_v)
            loss.backward()
